...
namespace Sky {
	...
	void OrthographicCameraController::OnUpdate(Timestep ts)
	{
		...
		else if (Input::IsKeyPressed(SKY_KEY_D))
			m_CameraPosition.x += m_CameraTranslationSpeed * ts;

		if (Input::IsKeyPressed(SKY_KEY_W))
			m_CameraPosition.y += m_CameraTranslationSpeed * ts;
		else if (Input::IsKeyPressed(SKY_KEY_S))
			m_CameraPosition.y -= m_CameraTranslationSpeed * ts;

		if (m_Rotation)
		{
			if (Input::IsKeyPressed(SKY_KEY_Q))
				m_CameraRotation += m_CameraRotationSpeed * ts;
			else if (Input::IsKeyPressed(SKY_KEY_E))
				m_CameraRotation -= m_CameraRotationSpeed * ts;
	...
	void OrthographicCameraController::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(SKY_BIND_EVENT_FN(OrthographicCameraController::OnMouseScrolled));
