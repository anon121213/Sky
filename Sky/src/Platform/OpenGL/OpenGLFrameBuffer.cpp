...
namespace Sky
{
		...
		: m_Specification(spec)
	{
		for (auto spec : m_Specification.Attachments.Attachments)
		{
			if (!Utils::IsDepthFormat(spec.TextureFormat))
				m_ColorAttachmentsSpecs.emplace_back(spec);
			else
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		{
			SKY_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Maximum of 4 color attachments supported!");
		...
		SKY_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
